...
abstract class BaseDao<T extends BaseEntity> implements IUcrdDao<T>, Initializable {
	...
	private List<Condition> additionalCondition = new ArrayList<>();
	...
	public int deleteBy(T condition) {
		List<Condition> conditions = additional(ConditionFilter.generateCondition(condition));
	...
	public List<T> find(T t, Page page) {
		...
		List<Condition> conditions = additional(ConditionFilter.generateCondition(t));
		conditions.add(new Condition("status", String.valueOf(Status.INVALID), Operation.NOT_EQUAL));
	...
	/**
	 * The additional conditions of operation.Will be clear after one operation.
	 *
	...
	/**
	 * Get the additional conditions and clear them.
	 *
	 * @return the additional condition list.
	 */
	private List<Condition> additional(List<Condition> target) {
		synchronized (mutex) {
			if (!additionalCondition.isEmpty()) {
				target.addAll(additionalCondition);
